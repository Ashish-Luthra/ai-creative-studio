...
import type { EmailDocument, EmailSection } from '@studio/types/email'
...
describe('Google Fonts injection', () => {
  ...
  it('FONT #5 — top 30 fonts export contains exactly 31 entries and all are strings', async () => {
    const { TOP_30_FONTS } = await import('@studio/lib/canvas/googleFonts')
  ...
  it('FONT #6 — all top 30 fonts are present in the full GOOGLE_FONT_FAMILIES list', async () => {
    const { TOP_30_FONTS, GOOGLE_FONT_FAMILIES } = await import('@studio/lib/canvas/googleFonts')
