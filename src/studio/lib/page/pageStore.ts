...
interface PageEditorState {
  ...
  /** Replace the document with agent-generated section copy. */
  applyGeneratedCopy: (mode: PageMode, title: string, blocks: Array<{ type: string; slots: Record<string, string> }>) => void
...
export const usePageStore = create<PageEditorState>((set, get) => {
  ...
  return {
    ...
    /**
     * Build a document from the agent's section copy.
     *
     * Structure, ids, colours and images come from makeDefaultBlock; only text
     * slots are overwritten. A section the model omitted simply doesn't appear,
     * and a slot it didn't fill keeps its placeholder — so a thin response
     * degrades instead of producing a broken page.
     */
    applyGeneratedCopy: (mode, title, blocks) => {
      const { document: doc, history } = get()
      const built = blocks
        .map((b) => {
          const base = makeDefaultBlock(b.type as PageBlockType)
          const content = { ...base.content }
          for (const [slot, value] of Object.entries(b.slots)) {
            const existing = content[slot]
            if (existing && existing.type === 'text') content[slot] = { type: 'text', value }
          }
          return { ...base, content }
        })
      ...
      const next: PageDocument = {
        ...createDefaultDocument(mode),
        title: title || doc.title,
        blocks: built,
      }
      set({ document: next, compiledHtml: compilePage(next), history: pushHist(history, doc), future: [], selectedBlockId: null })
    },
