...
| `role` | Semantic meaning — used for validation, analytics, and AI (future) |
| `content` | User-editable data only |
| `style` | Preset-driven visual system: typography, colors, spacing, button styles |
| `layout` | Spacing and alignment: margin, alignment |
| `behavior` | Interaction definitions: links, CTA actions, tracking |
| `meta` | System-level properties: variantGroup (experiments), locked, hidden |
 
**Supported roles:**
`heading` · `subheading` · `eyebrow` · `body` · `primary_cta` · `secondary_cta` · `supporting_image`
...
| Source | Behavior |
|---|---|
| `account` | Pulled from user profile — managed centrally via "Manage my logo" |
...
**Role:** Allow users to save any block or group of blocks as a named module, reuse it across emails, and update all instances from a single source.
...
| Step | Action |
|---|---|
| Save | User selects a block → clicks "Save block" in floating toolbar → names it → stored in library |
| Reuse | Saved blocks appear in the left insert panel under "Saved" category |
| Insert | Drag or click to insert — block renders with stored content and styles |
| Update | Editing a saved block prompts: "Update all uses?" or "Edit this copy only" |
| Scope | Saved blocks are per-account (not per-email) |
 
**Constraints:**
- Saved blocks do not carry section-level structure — blocks only, not full sections
- Max saved blocks: 50 per account (MVP)
- Names must be unique per account
- Deletion warns if block is in use in active emails
**Sidebar controls:** Name · Category tag · Replace · Delete
 
**Rendering:** Saved blocks compile identically to inline blocks — no special renderer needed.
...
- CTA Section
- Footer
> The 2-column section is the only multi-column layout in MVP. All other sections are single-column. Additional column configurations are out of scope for Phase 1.
...
- `src: null` means solid color only — no image rendered
- When `src` is set, `fallbackColor` is always required for Outlook compatibility
- Compiler renders VML background for Outlook + CSS background for all other clients
...
| Source | Behavior |
|---|---|
| `account` | Auto-applied across all emails via "Manage social links" |
| `custom` | Override per campaign |
...
| Benefit | Description |
|---|---|
| Faster creation | Users insert complete sections instead of assembling layouts manually |
| Design safety | All sections follow validated structures and constraints |
| Consistency | Spacing, typography, and hierarchy are standardized |
| Conversion optimization | Sections are designed for specific roles (Hero, CTA, Product) |
| Reduced cognitive load | Users choose from meaningful section types instead of raw blocks |
...
| Field | Description |
|---|---|
| `id` | Unique identifier for the section template |
| `label` | Human-readable name shown in the UI |
| `category` | Grouping in the insert panel |
| `structure` | Layout using Section → Row → Column → Block |
| `defaultContent` | Initial content on insert (text, images, CTA labels, links) |
| `constraints` | Structural and behavioral restrictions (maxBlocks, allowColumnChange, maxCTAs) |
| `editableFields` | What the user can modify (e.g. `headline.text`, `cta.link`) |
| `meta.variantGroup` | Supports experimentation and A/B testing |
        ...
        "columns": [
          {
            "width": "50%",
            "blocks": [
              { "type": "image", "role": "supporting_image" }
            ]
          },
          {
            "width": "50%",
...
> On mobile, the two columns stack vertically with image on top. `stackOrder` controls which column appears first. The compiler generates `@media` queries to handle this. Column widths are always 50-50 — not user-configurable in MVP.
...
| System | Relationship |
|---|---|
| Block System | Sections are composed of Blocks |
| CTA System | CTA behavior is embedded within section blocks |
| HTML Compiler | Sections are compiled into email-safe HTML |
| State Management | Sections exist as nodes within editor state |
...
| Background Rendering | If `section.styles.backgroundImage.src` is set: render VML for Outlook + CSS `background-image` for all other clients. Always render `fallbackColor` as `bgcolor` attribute on the `<td>` |
| Two-Column Layout | Render as two `<td>` elements inside a single `<tr>`. Apply `@media (max-width: 600px)` to stack columns and set each to `width: 100%` |
...
- Stack order on mobile is controlled by `section.responsive.stackOrder` — defaults to `image-first`
---
...
`savedBlocksStore` is a separate Zustand store. It fetches from the account-level saved blocks library on editor mount and persists changes on save.
